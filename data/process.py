...
class CROHME2Graph(object):
    ...
    def convert(self, lg_path):
        ...
        # print(graph['node_types'], type(graph['node_types']), graph['node_types'].dtype)
        #
        # objs = torch.tensor(graph['node_types']).long() # raw

        objs = torch.tensor(graph['node_types'].astype(np.int64)).long()  # @ym
