...
class CROHMELabelGraphDataset(Dataset):
    ...
    def __init__(self, root, names, nc, image_size=(256, 256)):
        ...
        self.lpls_dir = os.path.join(root, 'Train_imgslpls')
        self.latexcontent_dir = os.path.join(root, 'Latex_content_process')
        ...
        self.lpls_paths = [os.path.join(self.lpls_dir, name + '.png') for name in self.names]
        self.latexcontent_paths = [os.path.join(self.latexcontent_dir, name + '.png') for name in self.names]
    ...
    def __getitem__(self, index):
        ...
        lpls_path = self.lpls_paths[index]
        lpls = Image.open(lpls_path).convert('RGB')
        # lpls_64 = self.transform_64(lpls)
        # lpls_128 = self.transform_128(lpls)
        lpls_256 = self.transform_256(lpls)

        latexcontent_path = self.latexcontent_paths[index]
        latexcontent = Image.open(latexcontent_path).convert('RGB')
        # latexcontent_64 = self.transform_64(latexcontent)
        # latexcontent_128 = self.transform_128(latexcontent)
        latexcontent_256 = self.transform_256(latexcontent)
        ...
        return image_64, image_128, image_256, lpls_256, latexcontent_256, objs, boxes, layout, triples
...
def crohme_collate_fn(batch):
    ...
    all_lpls_256, all_latexcontent_256 = [], []
    all_obj_to_img, all_triple_to_img = [], []
    obj_offset = 0
    for i, (img_64, img_128, img_256, lpls_256, latexcontent_256, objs, boxes, layout, triples) in enumerate(batch):
        all_imgs_64.append(img_64[None])
        all_imgs_128.append(img_128[None])
        all_imgs_256.append(img_256[None])

        all_lpls_256.append(lpls_256[None])
        all_latexcontent_256.append(latexcontent_256[None])
    ...
    all_lpls_256 = torch.cat(all_lpls_256)
    all_latexcontent_256 = torch.cat(all_latexcontent_256)
    ...
    out = (all_imgs_64, all_imgs_128, all_imgs_256, all_lpls_256, all_latexcontent_256,
           all_objs, all_boxes, all_layout, all_triples,
           all_obj_to_img, all_triple_to_img)
    return out
