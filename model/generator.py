...
from model.fusion import Mix_TR
...
class Sg2ImModel(nn.Module):
    ...
    def __init__(self, vocab, image_size=(64, 64), embedding_dim=64,
                 ...
                 context_dim = 512, # ONE-DM adopt 512
                 **kwargs):
        ...
        # self.mix_net = Mix_TR(d_model=context_dim, num_encoder_layers=3, num_decoder_layers=3)

        self.mix_net = Mix_TR(d_model=context_dim, num_encoder_layers=1, num_decoder_layers=1)
    ...
    def forward(self, objs, triples, obj_to_img=None,
                img=None, lpls=None, latexcontent=None,
        ...
        # print(obj_vecs.shape) # 69, 128

        obj_vecs_orig = obj_vecs

        # print(p.shape) # 739
        pred_vecs = self.pred_embeddings(p)
        # print(pred_vecs.shape) # 739, 128
        ...
        # print(layout.shape) # 8 128 256 256
        ...
        mix_style = self.mix_net(img, lpls, latexcontent)

        if self.layout_noise_dim > 0:
            N, C, H, W = layout.size()
            noise_shape = (N, self.layout_noise_dim - 2, H, W)
            layout_noise = torch.randn(noise_shape, dtype=layout.dtype,
                                       device=layout.device)
            layout = torch.cat([layout, mix_style, layout_noise], dim=1)

        img_64, img_128, img_256 = self.refinement_net(layout)  # layout  8, 160, 256, 256
